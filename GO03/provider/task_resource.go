...
func (r *taskResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Make PUT request
	reqURL := "http://localhost:8080/tasks"
...
func (r *taskResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	task := Task{
		...
		Completed:   state.Completed.ValueBool(),
	}

        data, err := json.Marshal(task)

	if err != nil {
	    resp.Diagnostics.AddError("Foo", err.Error())
            return
	}

	reqURL := "http://localhost:8080/tasks"
	reqDel, _ := http.NewRequest(http.MethodDelete, reqURL, bytes.NewBuffer(data))
	client := &http.Client{}
	_, err2 := client.Do(reqDel)
	if err2 != nil {
		resp.Diagnostics.AddError("Delete Failed", err2.Error())
	}
}
